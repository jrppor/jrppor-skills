---
name: diagram
description: Architecture expert that draws Mermaid diagrams from input. Trigger when the user asks to draw, visualize, sketch, render, or create a diagram — including architecture diagrams, sequence diagrams, flowcharts, ER diagrams, state diagrams, class diagrams, user journeys, deployment diagrams, or C4 diagrams. Auto-selects the right diagram type from intent (structure for C4/architecture, time-ordered for sequence, decisions for flowchart, data model for ER, etc.), generates valid Mermaid code, validates the structure (orphan nodes, level consistency, size limits), critiques the architecture (SPOF, tight coupling, missing infrastructure), and suggests multiple views for complex systems. Output is Mermaid code (renders natively in Claude Code, GitHub, Obsidian, Notion).
---
